...
final Logger logger1 = new RootLogger(loggerConfig1);

configurable Config loggerConfig2 = {};
final Logger logger2 = new RootLogger(loggerConfig2);
...
function testBasicLoggingFunctions() returns io:Error? {
    ...
    string expectedMsg1 = string `, "level":"INFO", "module":"ballerina/log$test", "message":"This is an info message", "key1":"value1", "key2":"val:value2", "ctx":{"id":"ctx-1234", "msg":"Sample Context Message"}, "name":"logger1", "env":"prod"}`;
    ...
    test:assertTrue(logger1FileLogs[1].includes(expectedMsg21));
    test:assertTrue(logger1FileLogs[1].endsWith(expectedMsg22));
    test:assertTrue(logger1FileLogs[2].endsWith(expectedMsg3));
    ...
    string expectedMsg4 = string ` level=INFO module=ballerina/log$test message="This is an info message" name="logger2" env="dev"`;
