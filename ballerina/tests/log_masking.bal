...
function testLogMasking() returns error? {
    ...
    string expectedLog = string `"message":"user logged in","env":"test","user":{"name":"John Doe","password":"*****","mail":"joh**************com"}`;
    ...
    expectedLog = string `message="user logged in" env="test" user={"name":"John Doe","password":"*****","mail":"joh**************com"}`;
    test:assertEquals(maskedLogs.length(), 1);
    test:assertTrue(maskedLogs[0].includes(expectedLog));
    maskedLogs.removeAll();
    ...
    maskerJsonLogger.printInfo("user logged in", user = userTmp);
    expectedLog = string `"message":"user logged in","env":"test","user":{"name":"John Doe","ssn":"123-45-6789","password":"password123","mail":"john.doe@example.com","creditCard":"4111-1111-1111-1111"}`;
    test:assertEquals(maskedLogs.length(), 1);
    test:assertTrue(maskedLogs[0].includes(expectedLog));
    maskedLogs.removeAll();
    ...
    expectedLog = string `message="user login attempt failed" env="test" user={"name":"John Doe","password":"*****","mail":"joh**************com"}`;
    test:assertEquals(maskedLogs.length(), 1);
    test:assertTrue(maskedLogs[0].includes(expectedLog));
    maskedLogs.removeAll();

    maskerLogger.printInfo("basic types", str = "my string", num = 12345, flag = true, val = (), xmlVal = xml `<foo>bar</foo>`);
    expectedLog = string `message="basic types" env="test" str="my string" num=12345 flag=true val=null xmlVal=<foo>bar</foo>`;
    test:assertEquals(maskedLogs.length(), 1);
    test:assertTrue(maskedLogs[0].includes(expectedLog));
    maskedLogs.removeAll();
}
